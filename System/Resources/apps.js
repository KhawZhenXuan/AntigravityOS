(function () {
  ...
  function slides(container) {
    ...
    const renderSlideMedia = (slide, presenting = false) => {
      let layer=slide.querySelector("[data-slide-media]");if(!layer){layer=document.createElement("div");layer.dataset.slideMedia="";layer.style.cssText="display:flex;flex-wrap:wrap;gap:10px;justify-content:center;align-items:center;max-height:45%;overflow:auto";slide.insertBefore(layer,slide.querySelector("[data-slide-count]"))}layer.innerHTML="";
      (slidesData[active].media||[]).forEach((item,index)=>{const wrap=document.createElement("div");wrap.style.cssText="position:relative;max-width:45%;max-height:180px";const media=item.type.startsWith("video/")?document.createElement("video"):document.createElement("img");media.src=item.data;media.style.cssText="display:block;max-width:100%;max-height:170px;object-fit:contain";if(media.tagName==="VIDEO")media.controls=true;wrap.appendChild(media);if(!presenting){const remove=document.createElement("button");remove.className="app-button";remove.textContent="×";remove.title="Remove media";remove.style.cssText="position:absolute;right:2px;top:2px;padding:3px 7px";remove.onclick=()=>{remember();slidesData[active].media.splice(index,1);draw()};wrap.appendChild(remove)}layer.appendChild(wrap)});
    };
    ...
    const renderPresentedSlide = () => {
      ...
      renderSlideMedia(presentationSlide, true);
    ...
    const draw = () => {
      ...
      const mediaInput=document.createElement("input"),mediaButton=document.createElement("button");mediaInput.type="file";mediaInput.accept="image/*,video/*";mediaInput.multiple=true;mediaInput.hidden=true;mediaButton.className="app-button";mediaButton.textContent="Insert photo/video";toolbar.append(mediaButton,mediaInput);mediaButton.onclick=()=>mediaInput.click();mediaInput.onchange=async()=>{remember();const additions=await Promise.all([...mediaInput.files].map(file=>new Promise(resolve=>{const reader=new FileReader();reader.onload=()=>resolve({name:file.name,type:file.type,data:reader.result});reader.readAsDataURL(file)})));slidesData[active].media=[...(slidesData[active].media||[]),...additions];draw()};
  ...
  appByKey("docs").render = container => {
    ...
    const imageInput=document.createElement("input"),imageButton=document.createElement("button");imageInput.type="file";imageInput.accept="image/*";imageInput.multiple=true;imageInput.hidden=true;imageButton.className="app-button";imageButton.textContent="Insert photo";container.querySelector("[data-tools]").append(imageButton,imageInput);let savedRange=null;imageButton.onmousedown=()=>{const selection=getSelection();if(selection.rangeCount&&editor.contains(selection.anchorNode))savedRange=selection.getRangeAt(0).cloneRange()};imageButton.onclick=()=>imageInput.click();imageInput.onchange=()=>{[...imageInput.files].forEach(file=>{const reader=new FileReader();reader.onload=()=>{editor.focus();if(savedRange){const selection=getSelection();selection.removeAllRanges();selection.addRange(savedRange)}document.execCommand("insertImage",false,reader.result);editor.querySelectorAll("img").forEach(image=>image.style.cssText="max-width:100%;height:auto")};reader.readAsDataURL(file)})};
  ...
  appByKey("sheets").render = container => {
    const key=userKey("ag-sheets-v2"),rows=30,cols=12,data=JSON.parse(localStorage.getItem(key)||"{}");let selected="A1",clipboard=null,cutClipboard=false,clipboardSource=null,draggedCell=null;
    ...
    const colName=n=>String.fromCharCode(65+n); const calculated=(id,seen=new Set())=>{if(seen.has(id))return"#CYCLE!";seen.add(id);const raw=String(data[id]?.value||"");if(!raw.startsWith("="))return raw;try{let expression=raw.slice(1).replace(/(SUM|AVERAGE|MIN|MAX)\(([A-L]\d+):([A-L]\d+)\)/gi,(_,fn,start,end)=>{const column=start[0],from=Number(start.slice(1)),to=Number(end.slice(1)),values=[];for(let row=from;row<=to;row++)values.push(Number(calculated(`${column}${row}`,new Set(seen)))||0);return fn.toUpperCase()==="SUM"?values.reduce((a,b)=>a+b,0):fn.toUpperCase()==="AVERAGE"?values.reduce((a,b)=>a+b,0)/values.length:fn.toUpperCase()==="MIN"?Math.min(...values):Math.max(...values)}).replace(/\b[A-L]\d+\b/g,ref=>Number(calculated(ref,new Set(seen)))||0);if(!/^[0-9+\-*/().\s]+$/.test(expression))throw 0;return String(Function(`return (${expression})`)())}catch{return"#ERROR!"}}; const draw=()=>{container.innerHTML=`<h2 class="app-title">Sheets</h2><div class="button-row"><button class="app-button" data-new>New</button><button class="app-button" data-save>Save</button><button class="app-button" data-csv>Export CSV</button><button class="app-button" data-bold><b>B</b></button><input type="color" data-fill title="Cell fill"><input class="app-input" data-name value="${selected}" style="width:70px"><input class="app-input" data-formula placeholder="Value or =SUM(A1:A5)" style="flex:1;min-width:180px"></div><div style="overflow:auto;max-height:520px;margin-top:12px"><table class="info-table" style="min-width:900px"><thead><tr><th></th>${Array.from({length:cols},(_,c)=>`<th>${colName(c)}</th>`).join("")}</tr></thead><tbody>${Array.from({length:rows},(_,r)=>`<tr><th>${r+1}</th>${Array.from({length:cols},(_,c)=>{const id=`${colName(c)}${r+1}`,cell=data[id]||{};return`<td contenteditable data-cell="${id}" style="min-width:90px;background:${cell.fill||"transparent"};font-weight:${cell.bold?"700":"400"}">${esc(calculated(id))}</td>`}).join("")}</tr>`).join("")}</tbody></table></div>`;
      ...
      const selectCell=cell=>{selected=cell.dataset.cell;container.querySelectorAll("[data-cell]").forEach(item=>item.style.outline="none");cell.style.outline="2px solid var(--primary)";cell.style.outlineOffset="-2px";container.querySelector("[data-name]").value=selected;formula.value=data[selected]?.value||"";cell.focus()};
      const moveSelection=(columnDelta,rowDelta)=>{const match=selected.match(/^([A-L])(\d+)$/);if(!match)return;const column=Math.max(0,Math.min(cols-1,match[1].charCodeAt(0)-65+columnDelta)),row=Math.max(1,Math.min(rows,Number(match[2])+rowDelta));selectCell(container.querySelector(`[data-cell="${colName(column)}${row}"]`))};
      container.querySelectorAll("[data-cell]").forEach(cell=>{const saved=data[cell.dataset.cell]||{};cell.tabIndex=0;cell.draggable=true;if(saved.image){const image=document.createElement("img");image.src=saved.image;image.style.cssText="display:block;max-width:120px;max-height:90px;object-fit:contain;margin:4px auto";cell.appendChild(image)}cell.onclick=()=>selectCell(cell);cell.ondragstart=()=>{draggedCell=cell.dataset.cell};cell.ondragover=event=>event.preventDefault();cell.ondrop=event=>{event.preventDefault();if(!draggedCell||draggedCell===cell.dataset.cell)return;remember();data[cell.dataset.cell]={...(data[draggedCell]||{})};delete data[draggedCell];selected=cell.dataset.cell;draggedCell=null;draw()};cell.onkeydown=event=>{const keyName=event.key.toLowerCase();if((event.ctrlKey||event.metaKey)&&["c","x","v"].includes(keyName)){event.preventDefault();if(keyName==="c"||keyName==="x"){clipboard=JSON.parse(JSON.stringify(data[selected]||{}));cutClipboard=keyName==="x"}else if(clipboard){remember();data[selected]=JSON.parse(JSON.stringify(clipboard));if(cutClipboard&&draggedCell!==selected){cutClipboard=false}draw()}return}if(event.key==="ArrowLeft"){event.preventDefault();moveSelection(-1,0)}else if(event.key==="ArrowRight"){event.preventDefault();moveSelection(1,0)}else if(event.key==="ArrowUp"){event.preventDefault();moveSelection(0,-1)}else if(event.key==="ArrowDown"){event.preventDefault();moveSelection(0,1)}};});
      selectCell(container.querySelector(`[data-cell="${selected}"]`)||container.querySelector("[data-cell]"));
      ...
      const photoInput=document.createElement("input"),photoButton=document.createElement("button");photoInput.type="file";photoInput.accept="image/*";photoInput.hidden=true;photoButton.className="app-button";photoButton.textContent="Insert photo";toolbar.append(photoButton,photoInput);photoButton.onclick=()=>photoInput.click();photoInput.onchange=()=>{const file=photoInput.files[0];if(!file)return;const reader=new FileReader();reader.onload=()=>{remember();data[selected]={...(data[selected]||{}),image:reader.result};draw()};reader.readAsDataURL(file)};
